...
* 通过设置 `SHARD_ROW_ID_BITS` ([https://github.com/pingcap/docs/blob/master/sql/tidb-specific.md#shard_row_id_bits](https://github.com/pingcap/docs/blob/master/sql/tidb-specific.md#shard_row_id_bits))，可以把 rowid 打散写入多个不同的 Region，缓解写入热点问题：`ALTER TABLE table_name SHARD_ROW_ID_BITS = 8;`。
...
于是我们选择了一个折中的方案，也是目前 TiDB 比较推荐的方案：单台物理机部署多个 TiDB 实例，通过端口进行区分，给不稳定查询的端口设置内存限制（如图 5 中间部分的 TiDBcluster1 和 TiDBcluster2）。
